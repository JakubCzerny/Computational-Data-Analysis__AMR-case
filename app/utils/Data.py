...
class Data(object):
    ...
    def get_gene_count_with_drugs(self, cut_off=None):
        ...
        Y = np.array([drugs_per_country[sample_country[int(code)]].values for code in self.gene_counts_df.columns[1:]])
        X = self.gene_counts_df.ix[:, self.gene_counts_df.columns[1:]].transpose().as_matrix()

        if cut_off:
            indices = np.where(X.sum(axis=0) < cut_off)[0]
